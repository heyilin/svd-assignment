...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector scores) {
        ...
        int userIdx = model.getUserIndexMapping().getIndex(user);
        double features = model.getItemFeatureMatrix().getColumnDimension();
        for (VectorEntry e: scores.fast(VectorEntry.State.EITHER)) {
            double result = 0;
            long item = e.getKey();
            // TODO Set the scores
            int itemIdx = model.getItemIndMapping().getIndex(item);
            for (int i = 0;i<features;i++){
                double uaf = userVector.getEntry(userIdx,i);
                double vif = model.getItemFeatureMatrix().getEntry(itemIdx,i);
                double featureWeight = model.getFeatureWeights().getEntry(i,i);
                result += uaf*featureWeight*vif;
            }
            scores.set(item,result);
        }
    }
